package com.apesRise.hotPointer.util;

public class Constant {
	
	public final static int META = 1;
	
	public final static int APPROVED = 2;
	
	public final static int UNRELATED = 3;
	
	public final static int DELICIOUS = 4;
	
	public final static int HEALTHY = 5;
	
	public final static String UNRELATED_DIR = "train/unrelated/";
	
	public final static String APPROVE_DIR = "train/approve/";
	
	public final static String META_DIR = "train/delta/";
	
	public final static String KNN_COOK_PROPERTY_FILE = "KnnCookProperties.txt";
	
	public final static String KNN_DELICIOUS_PROPERTY_FILE = "KnnDeliciousProperties.txt";

	
	public final static String KNN_HEALTHY_PROPERTY_FILE = "KnnHealthyProperties.txt";

}
